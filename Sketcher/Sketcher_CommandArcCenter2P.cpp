#include "Sketcher_CommandArcCenter2P.h"
...
Standard_Boolean Sketcher_CommandArcCenter2P::MouseInputEvent(const gp_Pnt2d& thePnt2d)
{
	...
	case Input_CenterArc:
	{
		...
		myContext->Display(myRubberCircle, 0, -1, Standard_True);

		myRubberCenterPoint->SetComponent(myFirstPoint);
		myRubberLine->SetPoints(myFirstPoint, myFirstPoint);
		myContext->Display(myRubberCenterPoint, 0, -1, Standard_True);
		myContext->Redisplay(myRubberCenterPoint, Standard_True);
	...
	case Input_2ArcPoint:
		...
		if (ProjectOnCircle(curPnt2d))
		{
			...
			if (tempGcc_Circ2d3Tan.IsDone() && tempGcc_Circ2d3Tan.NbSolutions()>0)
			{
				...
				myContext->Remove(myRubberCircle, Standard_True);
				myContext->Remove(myRubberCenterPoint, Standard_True);
...
void Sketcher_CommandArcCenter2P::CancelEvent()
{
	...
	case Input_1ArcPoint:
	case Input_MidPoint:	myContext->Remove(myRubberLine, Standard_True);

	case Input_2ArcPoint:	myContext->Remove(myRubberCircle, Standard_True);
		myContext->Remove(myRubberCenterPoint, Standard_True);
		break;
	default: break;
	}
