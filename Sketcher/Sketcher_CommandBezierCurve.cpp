...
Standard_Boolean Sketcher_CommandBezierCurve::MouseInputEvent(const gp_Pnt2d& thePnt2d)
{
	...
	case Input_2Point:
	{
		...
		if (ME.IsDone())
		{
			...
			myContext->Display(myRubberAIS_Shape, 0, -1, Standard_True);
	...
	case Input_OtherPoints:
	{
		...
		if (ME.IsDone())
		{
			...
			{
				myRubberAIS_Shape->Set(curEdge);
				myContext->Redisplay(myRubberAIS_Shape, Standard_True);
