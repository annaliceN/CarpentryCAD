...
void Sketcher_SnapLineParallel::DrawRelation()
{
	...
	if (firstDisplay) myContext->Display(myParRelation, 0, -1, Standard_True);
	else myContext->Redisplay(myParRelation, Standard_True);
}
...
{
	myContext->Remove(myParRelation, Standard_True);
}