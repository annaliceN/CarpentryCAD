...
void Sketcher_SnapLinePerpendicular::DrawRelation()
{
	...
	if (firstDisplay) myContext->Display(myPerRelation, 0, -1, Standard_True);
	else myContext->Redisplay(myPerRelation, Standard_True);
}
...
{
	myContext->Remove(myPerRelation, Standard_True);
}
