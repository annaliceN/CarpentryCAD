...
gp_Pnt2d Sketcher_Snap::MouseMoveEvent(const gp_Pnt2d& tempPnt2d)
{
	...
	if (findbestPnt2d)
	{
		...
		if (firstDisplay)
		{
			myContext->Display(myAIS_Point, 0, -1, Standard_True);
			DrawRelation();
			firstDisplay = Standard_False;
		}
		else
		{
			myContext->Redisplay(myAIS_Point, Standard_True);
			DrawRelation();
		}
	}
	else
	{
		myContext->Remove(myAIS_Point, Standard_True);
		EraseRelation();
		firstDisplay = Standard_True;
	}
...
{
	myContext->SetSelected(curHilightedObj, Standard_True);
}
